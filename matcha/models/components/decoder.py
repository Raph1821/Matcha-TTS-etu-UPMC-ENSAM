    ...
    def __init__(  # pylint: disable=useless-super-delegation
        ...
        *,
        ...
        if not CONFORMER_AVAILABLE:
            raise ImportError("conformer package is required for ConformerWrapper")
        super().__init__(
            dim=dim,
            dim_head=dim_head,
            heads=heads,
...
class Decoder(nn.Module):
    ...
    def get_block(self, block_type, dim, attention_head_dim, num_heads, dropout, act_fn):
        if block_type == "conformer":
            if not CONFORMER_AVAILABLE:
                raise ImportError("conformer package is required for conformer blocks")
            ...
            if not DIFFUSERS_AVAILABLE:
                raise ImportError("diffusers package and BasicTransformerBlock are required for transformer blocks")
