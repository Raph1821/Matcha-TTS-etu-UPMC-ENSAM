...
- [Optimisation](#optimisation-cython)
...
Pour avoir plus d'informations, veuillez consulter le [README_CYTHON](guide_book/README_CYTHON.md)
...
# 2. Compiler Cython (optionnel mais recommandé)
python compiler_cython.py

# 3. Lancer l'entraînement
python train.py

# 4. Reprendre l'entraînement si interrompu
python train.py  # Reprend automatiquement

# 5. Forcer un nouveau départ
...
# Ouvrir les notebooks de test :
# - notebooks/test_text.ipynb
# - notebooks/test_audio_to_Mel.ipynb
# - test_pipeline.ipynb
```
...
**Pour tester les gains :**
```bash
python test_monotonic_align_speed.py
```

Voir [README_CYTHON.md](guide_book/README_CYTHON.md) pour plus de détails.
...
1. **Entraîner** le modèle sur vos données
2. **Analyser** les métriques pour vérifier la convergence
3. **Générer** de l'audio avec différents textes
4. **Optimiser** les hyperparamètres si nécessaire
5. **Expérimenter** avec différentes configurations

Pour plus de détails techniques, consultez [ARCHITECTURE_PROTOCOL.md](guide_book/ARCHITECTURE_PROTOCOL.md).
